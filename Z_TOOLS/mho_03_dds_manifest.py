"""
MHO_03 DDS MANIFEST - PSD/PNG to DDS Batch Exporter with Manifest for Marvel Heroes Omega
...
FUNCTIONALITY
  ...
  Can run standalone as a Tkinter GUI, or be imported by mho_01_mod_selector.py
  which delegates to process_folder(), write_manifest(), copy_to_target(),
  browse_texconv(), and browse_target_folder().

  Two export actions:
    - EXPORT DDS     : converts source images to DDS, writes manifest, optionally
...
KEY COMPONENTS
  ...
  - main()                : standalone Tkinter GUI (folder list, settings, log)
...
def main():
    ...
    tk.Checkbutton(
        frame, text='Ignore source images starting with "00_"', variable=ignore_00_var,
        bg=BG, fg=FG, selectcolor=ENTRY_BG, activebackground=BG, activeforeground=FG
    ...
    target_checks_frame.grid(row=6, column=0, columnspan=3, sticky=tk.W, pady=2)
    tk.Checkbutton(
    ...
    tk.Label(frame, text="Target Folder:", bg=BG, fg=FG).grid(row=7, column=0, sticky=tk.W, pady=2)
    target_entry = tk.Entry(frame, width=50, bg=ENTRY_BG, fg=FG, insertbackground=FG)
    target_entry.grid(row=7, column=1, sticky=tk.EW, pady=2, padx=5)
